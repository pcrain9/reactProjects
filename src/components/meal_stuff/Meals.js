...
function Meals(props) {
  ...
  function addItemHandler(event) {
    ...
    if (
      amountEntered === "0" ||
      numberAmountEntered < 1 ||
      numberAmountEntered > 5
    ) {
      return;
    }
  ...
  return (
    <Card className={classes.meal}>
      <form className={classes.userInteractItems} onSubmit={addItemHandler}>
        <label className={classes.amount} htmlFor={classes.amount}>
          How many?{" "}
        </label>
        <input
      ...
      <span className={classes.description_container}>
        <h2>{props.name}</h2>
        <img className={classes.image} src={props.image} alt="null" />
      </span>
